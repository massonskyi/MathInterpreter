...
private:
    ...
    void M_function_execute(const std::string &command)
    {
        ...
        else
        {
            ...
            auto x = M_function_expression(command, "");
    ...
    ASTValue evaluate_ast_node(std::shared_ptr<ASTNode> node)
    {
        return std::visit([](auto &&arg) -> ASTValue
            ...
            if constexpr (std::is_same_v<NodeType, Variable>) {
                return arg;  // Если это просто переменная, возвращаем её значение
            } else if constexpr (std::is_same_v<NodeType, Matrix>) {
                return arg;  // Если это матрица, возвращаем её значение
            } else if constexpr (std::is_same_v<NodeType, Vector>) {
                return arg;  // Если это вектор, возвращаем его значение
            } else if constexpr (std::is_same_v<NodeType, Rational>) {
                return arg;  // Если это рациональное число, возвращаем его значение
            } else {
    ...
    void _M_print_variable(const std::string &variableName)
    {
        ...
        if (!ctx.findVariable(var_without_whitespace))
        {
            throw UnknownVariable("Variable not found: " + var_without_whitespace);
        }

        std::cout << ctx.getVariable(var_without_whitespace) << std::endl;
    }
