...
#include "parsers/functionparser.hxx"

#include "function/base.hxx"

#include "math/binaryexecuter.hxx"
#include "math/expressionparser.hxx"

#include "io/ioreader.hxx"
...
public:
    ...
    : _M_binary_executer()
    {
        ...
        append_(_M_regexes, "MatrixLiteral",        std::regex(R"(\[\s*(\[\s*[a-zA-Z\d]+\s*(?:,\s*[a-zA-Z\d]+\s*)*\]\s*(?:,\s*\[\s*[a-zA-Z\d]+\s*(?:,\s*[a-zA-Z\d]+\s*)*\]\s*)*)\])"));
        append_(_M_regexes, "VectorLiteral",        std::regex(R"(^\[\s*([a-zA-Z\d]+\s*,\s*)*[a-zA-Z\d]+\s*\]$)"));
    ...
    void processConsole()
    {
        ...
        while (true)
        {
            ...
            // Check for multi-line input (e.g., function declaration)
            if (line.rfind("fn", 0) == 0) { // Check if the line starts with "fn"
                ...
                try {
                    FuncParser parser;
                    FuncParser::NewFunction func = parser.parse(multiLineInput);
                    std::cout << "Function parsed successfully:\n";
                    std::cout << "Name: " << func._func_name << "\n";
                    std::cout << "Return Type: " << func._return_type << "\n";
                    std::cout << "Arguments:\n";
...
private:
    ...
    using stack_variables = std::unordered_map<std::string, value_type>;

    std::unordered_map<std::string, std::regex> _M_regexes; 

    stack_variables _M_variables{};
    stack_functions _M_functions{};
    ...
    [[nodiscard]] value_type _M_parse_rational_expression(const std::string &expression, const std::string &op)
    {
        ...
        else if(std::regex_match(expression, match, _M_regexes.find("RationalOperation")->second))
        {
            return _M_binary_executer.calculate(expression, _M_variables);
        }
        throw InvalidSyntax("Invalid rational number or operation");
    }
    ...
    value_type _M_parse_matrix_expression(const std::string &expression, const std::string &op)
    {
        ...
        {
            auto expr = _M_binary_executer.replace_variables(expression, _M_variables);
            return make_matrix_ptr(_M_parse_matrix(expr));
        }
        else
        {
            return _M_binary_executer.calculate(expression, _M_variables);
    ...
    value_type _M_parse_vector_expression(const std::string &expression, const std::string &op)
    {
        ...
        {
            auto expr = _M_binary_executer.replace_variables(expression, _M_variables);
            return make_vector_ptr(Vector(_M_parse_vector(expr)));
        }
        else
        {
            return _M_binary_executer.calculate(expression, _M_variables);
