...
public:
    ...
    {
        std::stringstream ss;
        ss << "Matrix of Vector " << data_.at(0).at(0).getType() << " with size " << data_.size() << ": [\n";
    ...
    inline size_t cols() const { return data_.empty() ? 0 : data_[0].size(); }

    // Access operations
    inline Vector &operator[](size_t index) { return data_[index]; }
    inline const Vector &operator[](size_t index) const { return data_[index]; }

    inline Vector &at(size_t index) { return data_.at(index); }
    inline const Vector &at(size_t index) const { return data_.at(index); }

    inline Variable get(size_t row, size_t col) const
    {
        return data_.at(row).get(col);
    }

    // Modifiers
    inline void push_back(const Vector &row)
    {
